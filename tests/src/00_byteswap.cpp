...
TEST(ECPP_Endian, Bswap) {
    using ecpp::impl::bswap;
    constexpr std::uint8_t x8 = 0x12U;
    static_assert(bswap(x8) == x8);
    EXPECT_EQ(bswap(x8), x8);

    constexpr std::uint16_t x16 = 0x1122U;
    static_assert(bswap(x16) == 0x2211U);
    EXPECT_EQ(bswap(x16), 0x2211U);

    constexpr std::uint32_t x32 = 0x11223344U;
    static_assert(bswap(x32) == 0x44332211U);
    EXPECT_EQ(bswap(x32), 0x44332211U);


    constexpr std::uint64_t x64 = 0x1122334455667788U;
    static_assert(bswap(x64) == 0x8877665544332211U);
    EXPECT_EQ(bswap(x64), 0x8877665544332211U);
}
